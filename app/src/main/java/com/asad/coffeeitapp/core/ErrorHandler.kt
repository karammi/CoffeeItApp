...
class ApiRunner @Inject constructor(
    ...
    suspend fun <T> invokeSuspended(job: suspend () -> T): Result<T> {
        return try {
            ...
            Result.Error(errorHandler.convertToApiErrorBody(e))
        }
    }

    fun <T> invoke(job: () -> T): Result<T> {
        return try {
            Result.Success(job.invoke())
        } catch (e: Exception) {
            Result.Error(errorHandler.convertToApiErrorBody(e))
...
sealed class Result<out R> {
    ...
    data class Error(val apiErrorBody: ApiErrorBody) : Result<Nothing>()
    object Loading : Result<Nothing>()

    override fun toString(): String {
        return when (this) {
            is Success<*> -> "Success[data=$data]"
            is Error -> "Error[exception=${apiErrorBody.message}]"
            Loading -> "Loading"
        }
    }
}
